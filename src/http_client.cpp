...
HttpResponse HttpClient::get(const std::string& url, 
    ...
    try {
        ...
        if (is_https) {
            ...
            res = client.Get(path.c_str(), req_headers);
#else
            response.error_message = "HTTPS not supported - OpenSSL required";
            return response;
#endif
        } else {
...
HttpResponse HttpClient::post(const std::string& url, 
    ...
    try {
        ...
        if (is_https) {
            ...
            client.set_follow_location(true);
            client.enable_server_certificate_verification(false);
            res = client.Post(path.c_str(), req_headers, body, "application/json");
#else
            response.error_message = "HTTPS not supported - OpenSSL required";
            return response;
#endif
